...
ניהול מערכות תובלה ושינוע, שם נבדקו הקולות, הניקוד וקצב ההפסקות.
"""
import asyncio
import hashlib
import html
...
COURSE = "שיטות וכלי ניהול מתקדמים"
...
PLANNED = [
    ("01", "מבוא לניהול — פונקציות, דרגים ומיומנויות"),
    ("02", "תכנון — יעדים, SWOT מתקדם ותרחישים"),
    ("03", "ארגון — מבנים, סמכות וביזור"),
    ("04", "מנהיגות — מהשריג ועד אינטליגנציה רגשית"),
...
# ערכת הנושא (amq_theme) זהים לבוחן ולמילון, כך שמצב כהה/בהיר עובר בין הדפים.
...
<script>try{var t=localStorage.getItem('amq_theme');if(t)document.documentElement.className=t;}catch(e){}</script>
<style>
...
<p class="sub">פרק לכל נושא, בשני קולות. בסוף כל פרק: שאלות לדרך, עם שקט קצר לחשוב לפני התשובה.</p>
...
<a href="../%D7%9E%D7%99%D7%9C%D7%95%D7%9F-%D7%9E%D7%95%D7%A9%D7%92%D7%99%D7%9D.html">📖 <span><b>מילון מושגים</b> <em>· כל המונחים לפי שיעור</em></span></a>
</div>
...
הקולות מסונתזים, והתסריטים נכתבו מאותו חומר של הבוחן והמילון, כדי שכולם יגידו אותו דבר.</p>
</div>
<script>
(function(){
  var audios=[].slice.call(document.querySelectorAll('audio'));
  var btns=[].slice.call(document.querySelectorAll('.speed button'));
  var rate=1;try{rate=parseFloat(localStorage.getItem('amq_rate'))||1;}catch(e){}
  function setRate(r){rate=r;audios.forEach(function(a){a.playbackRate=r;});
    ...
    try{localStorage.setItem('amq_rate',String(r));}catch(e){}}
